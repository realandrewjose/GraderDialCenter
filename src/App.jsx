...
function letterGrade(pct) {
  if (pct >= 93) return { letter: "A",  color: "#10b981" };
  if (pct >= 90) return { letter: "A−", color: "#10b981" };
  if (pct >= 87) return { letter: "B+", color: "#3b82f6" };
  if (pct >= 83) return { letter: "B",  color: "#3b82f6" };
  if (pct >= 80) return { letter: "B−", color: "#3b82f6" };
  if (pct >= 77) return { letter: "C+", color: "#f59e0b" };
  if (pct >= 73) return { letter: "C",  color: "#f59e0b" };
...
function InformativeSpeechGrader({ onBack }) {
  ...
  const gradeColor = pct >= 93 ? "#6ee7b7" : pct >= 83 ? "#fbbf24" : pct >= 73 ? "#fb923c" : pct > 0 ? "#f87171" : "#6b7280";
  ...
  const buildSummary = () => {
    ...
    t += `── TOTAL: ${totalScore}/${INFORMATIVE_TOTAL_PTS} (${pct}%) ──\n`;
    ...
    <div className="grader-wrap" style={{ fontFamily: "'Outfit', sans-serif", background: "#07090f", minHeight: "100vh", color: "#e2e8f0" }}>
      ...
      <div className="no-print" style={{ maxWidth: 880, margin: "0 auto", padding: "20px 16px 100px" }}>
        ...
        <div style={{
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 11, color: "#475569", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 6 }}>Percentage</div>
            <div style={{ display: "flex", alignItems: "center", gap: 14, justifyContent: "flex-end" }}>
              <div>
                <div style={{ fontSize: 22, fontWeight: 700, color: "#cbd5e1", letterSpacing: "-0.02em", marginBottom: 8 }}>{pct}%</div>
      ...
      <div className="print-only" style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: "11pt", color: "#000", lineHeight: 1.5 }}>
        ...
        <div style={{ borderTop: "2.5px solid #000", paddingTop: 10, marginTop: 6, display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
          ...
          <div style={{ fontFamily: "Courier New, monospace", fontSize: "18pt", fontWeight: 700 }}>
...
function PersuasiveSpeechGrader({ onBack }) {
  ...
  const buildSummary = () => {
    ...
    t += `── TOTAL: ${totalScore}/${PERSUASIVE_TOTAL_PTS} (${pct}%) ──\n`;
    if (additionalComments) t += `\nAdditional Comments:\n${additionalComments}\n`;
    return t;
  };
...
function SymposiumGrader({ onBack }) {
  ...
  const handleCopySummary = () => {
    ...
    text += `━━ TOTAL: ${totalScore}/${TOTAL_PTS} (${totalPct}% · ${grade.letter}) ━━\n`;
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, justifyContent: "flex-end" }}>
              ...
              <div>
                <div style={{ fontSize: 22, fontWeight: 700, color: "#cbd5e1",
                              letterSpacing: "-0.02em", marginBottom: 8 }}>{totalPct}%</div>
